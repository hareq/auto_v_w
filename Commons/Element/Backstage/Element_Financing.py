...
class financing:
    ...
    def financing_t(self,url,uername,purpose,pro_type,bank,moneyf,dayormauthtype,dayormauth,rate,end_time,service_fee,deposit,activity_a,activity_b,estate_name,build_size,limit,p_price,c_price,activity_url,driver):
      #url = url+"lender"
      #print url
      moneyf = int(moneyf)
      ...
      if activity_b == 'yes':
           driver.find_element_by_xpath("(//input[@name='activity_id[]'])[2]").click()
           #Select(driver.find_element_by_name("activity_id")).select_by_visible_text(u"2014年11月投标即送")
      if activity_b == 'no':
          1 == 1
           #Select(driver.find_element_by_name("activity_id")).select_by_visible_text(u"非活动标")
      driver.find_element_by_name("activity_url").send_keys(activity_url)
      if deposit == "是":
          driver.find_element_by_xpath("(//div[@id='uniform-day_click']/span)[2]").click()
      if deposit == "否":
          driver.find_element_by_xpath("(//div[@id='uniform-month_click']/span)[2]").click()
    ...
    def financing_z(self,url,uername,purpose,pro_type,bank,moneyf,dayormauthtype,dayormauth,rate,end_time,service_fee,deposit,activity_a,activity_b,estate_name,build_size,limit,p_price,c_price,activity_url,driver):
      #url = url+"lender"
      #print url
      moneyf = int(moneyf)
      dayormauth = int(dayormauth)
